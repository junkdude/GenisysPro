...
class PigZombieAI{
	...
	public function __construct(AIHolder $plugin){
		$this->plugin = $plugin;
		if($this->plugin->getServer()->aiConfig["pigzombie"]){
	...
	public function array_clear() {
		if (count($this->plugin->pigzombie) != 0) {
			foreach ($this->plugin->pigzombie as $eid=>$info) {
				foreach ($this->plugin->getServer()->getLevels() as $level) {
					if (!($level->getEntity($eid) instanceof Entity)) {
						unset($this->plugin->pigzombie[$eid]);
						//echo "清除 $eid \n";
					}
				}
			}
			...
			foreach ($level->getEntities() as $zo){
				if($zo instanceof Zombie){
					if ($this->plugin->willMove($zo)) {
						if (!isset($this->plugin->pigzombie[$zo->getId()])){
							$this->plugin->pigzombie[$zo->getId()] = array(
								'ID' => $zo->getId(),
								'IsChasing' => false,
								'motionx' => 0,
								'motiony' => 0,
							...
							$zom = &$this->plugin->pigzombie[$zo->getId()];
							$zom['x'] = $zo->getX();
							$zom['y'] = $zo->getY();
							$zom['z'] = $zo->getZ();
						}
